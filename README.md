...
Przed każdym krokiem autowalk sprawdza ostatnie `mv/max_mv` z `Char.Vitals`. Przy poziomie 10% lub niższym i włączonej opcji **Używaj refreshy** rzuca `refresh` na siebie, jeśli gotowy czar znajduje się w `Char.MemSpell`; w przeciwnym razie wysyła `rest`. Opcja **Używaj recuperate** dodaje komendę `recuperate` bezpośrednio po `rest`. Autowalk czeka najwyżej 30 sekund, ale jeśli w tym czasie `refresh` zostanie zapamiętany, natychmiast wstaje, rzuca gotowy `float`, następnie `refresh` i wznawia trasę. Obie opcje są domyślnie wyłączone i zapisywane w ustawieniach aplikacji. Gdy GMCP `Char.Condition` zgłosi `position: POS_SITTING`, autowalk wysyła `stand` i czeka z następnym krokiem na potwierdzenie `POS_STANDING`. Zatrzymanie autowalku anuluje oczekiwanie.
