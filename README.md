...
Ta sama zakładka **Automaty → Podróż** ma też automaty drużynowe, aktywne tylko wtedy, gdy jesteś liderem aktualnej grupy GMCP (komenda `order` działa tylko dla lidera): przycisk **Rozkaż drużynie rzucić refresh** wysyła `order <gracz> cast refresh` do każdego innego (nie-NPC) członka drużyny po kolei, a przełączniki **Autostand**/**Autosit** robią to samo z `order <gracz> stand` / `order <gracz> sit` automatycznie za każdym razem, gdy Ty wstajesz albo siadasz.
