...
Przed każdym krokiem autowalk sprawdza ostatnie `mv/max_mv` z `Char.Vitals`. Przy poziomie równym lub niższym od skonfigurowanego progu (domyślnie 10%) rzuca `refresh` na siebie, jeśli gotowy czar znajduje się w `Char.MemSpell`; w przeciwnym razie wysyła `rest`, czeka skonfigurowaną liczbę sekund (domyślnie 30) i wznawia trasę. Próg i czas odpoczynku można zmienić w **Automaty → Podróż**, tam też jest opcja **Rzuć refresh na drużynę**, która rzuca `refresh` po kolei na każdego innego (nie-NPC) członka drużyny. Gdy GMCP `Char.Condition` zgłosi `position: POS_SITTING`, autowalk wysyła `stand` i czeka z następnym krokiem na potwierdzenie `POS_STANDING`. Zatrzymanie autowalku anuluje oczekiwanie.
